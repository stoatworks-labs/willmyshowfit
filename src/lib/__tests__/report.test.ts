...
describe('report options', () => {
  ...
  it('signs every report, however trimmed', () => {
    // The attribution is not part of ReportSections and must not become so: a
    // report that leaves the building has to say what made it and whose
    // trademarks it is using.
    const bare: ReportOptions = {
      devices: [],
      sections: {
        inputList: false,
        outputList: false,
    ...
    for (const o of [undefined, defaultReportOptions(results), bare]) {
      const html = render(o)
      expect(html).toMatch(/Generated by <a href="https:\/\/willmyshowfit\.com">/)
      expect(html).toMatch(/Stoatworks Labs/)
      expect(html).toMatch(/All trademarks belong to their owners/)
      expect(html).toMatch(/— v\d+\.\d+\.\d+\./)
    }
  })

  it('carries the attribution into the downloaded file too', () => {
    expect(standaloneReportHtml(show, results, AT)).toMatch(/class="genfooter"/)
  })
